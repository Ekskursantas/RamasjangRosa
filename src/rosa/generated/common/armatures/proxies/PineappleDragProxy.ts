...
export class PineappleDragProxy extends ProxyBase implements IArmatureProxy,IDragable {
	...
	constructor(armature:dragonBones.Armature) {
		super();
		this.proxyName = "PineappleDragProxy";
		...
		this.rectCover.drawRect(-this.display.width*.5, -this.display.height*.25, this.display.width+this.display.width*.5, this.display.height+this.display.height*.5);
	...
	public getDecoArmature = ():dragonBones.Armature => {
		let armature:dragonBones.Armature = AssetLoader.getInstance().typeddb._factory.buildArmature("deco/pineappleDrag");
		return armature;
	}
